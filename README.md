...
![Version](https://img.shields.io/badge/version-0.36.0-informational)
...
The pipeline follows the build order: `schemas` → `fetch` → `preprocess` → `detect` → `link` → `classify` → `orbit` → `score` → `alert` → `calibration`. Each stage consumes the immutable, typed output of all prior stages. As of v0.36.0, all ten pipeline modules plus background automation are complete with 1354 tests, 100% code coverage, automated offline scheduling readiness, provider-specific live readiness summaries, no-network live dry-run approval bundles, live policy contract validation, persisted readiness checks and approval bundles, no-network live dry-run planning, mock-only provider execution attempt logs, and validated injection-recovery baselines of 100% detection, link, and score rates on $n = 200$ synthetic NEO tracklets.
...
```
┌─────────────────────────────────────────────────────────────────────┐
│                    NEO DETECTION PIPELINE  v0.36.0                  │
└─────────────────────────────────────────────────────────────────────┘
...
│   ├── __init__.py               # Package version (0.36.0)
...
│
├── tests/                        # pytest suite (1354 tests; 100% coverage)
...
| 8. Full test suite passes | `pytest` | 1354 / 1354 tests pass |
...
# Expected: 1354 passed in ~N seconds
```
...
| Metric | Definition | v0.36.0 Baseline |
|---|---|---|
...
### 12.4 Module Coverage Summary (v0.36.0)
