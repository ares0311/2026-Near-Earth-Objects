...
![Version](https://img.shields.io/badge/version-0.90.18-informational)
...
The pipeline follows the build order: `schemas` -> `fetch` -> `preprocess` -> `detect` -> `link` -> `classify` -> `orbit` -> `score` -> `alert` -> `calibration`. Each stage consumes the immutable, typed output of all prior stages. As of v0.90.18, the WISE/DECam/TESS path is a preserved secondary path with closed historical gates, while the primary development path is ZTF DR24 archival historical replay per `docs/MISSION.md` and `docs/neo_discovery_agent_brief.md`. Phase 0 source verification is recorded in `docs/evidence/phase0/`: JPL SBDB, MPC get-obs, and IRSA ZTF metadata are live-verified; Fink remains externally blocked at TLS; pretrained model use is deferred. The ZTF DR24 production gates and Phase 1 historical-replay prototype are the next work. External MPC submission remains fail-closed until a real candidate survives review and the applicable source/submission protocol is satisfied.
...
```
┌─────────────────────────────────────────────────────────────────────┐
│                    NEO DETECTION PIPELINE  v0.90.18                  │
└─────────────────────────────────────────────────────────────────────┘
...
│   ├── __init__.py               # Package version (0.90.18)
...
### 15.1 Current State Snapshot (v0.90.12)
...
| Production capability gates | **P1-P3, P5 closed; P4 open (dormant)** | P1, P2 (`docs/SURVEY_NATIVE_CONFIDENCE_POLICY.md`), P3 (`docs/evidence/prod-loop/2026-07-02-gate-p3-no-submission-drill.md`), and P5 (`docs/OPERATOR_GO_NO_GO_RUNBOOK.md`) are closed. P4 MPC submission protocol is the only remaining open production gate; it is dormant, not an active task — no real candidate exists yet, so there is nothing to contact MPC about. It activates only once a real WISE-sourced candidate survives adversarial and operator review. Actual discovery is an event gate, not a production prerequisite. |
