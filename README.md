...
The pipeline follows the build order: `schemas` → `fetch` → `preprocess` → `detect` → `link` → `classify` → `orbit` → `score` → `alert` → `calibration`. Each stage consumes the immutable, typed output of all prior stages. As of v0.27.0, all ten pipeline modules plus background automation are complete with 1335 tests, 100% code coverage, automated offline scheduling readiness, persisted readiness checks, and validated injection-recovery baselines of 100% detection, link, and score rates on $n = 200$ synthetic NEO tracklets.
...
```
┌─────────────────────────────────────────────────────────────────────┐
│                    NEO DETECTION PIPELINE  v0.27.0                  │
└─────────────────────────────────────────────────────────────────────┘
...
│   ├── __init__.py               # Package version (0.27.0)
...
│
├── tests/                        # pytest suite (1335 tests; 100% coverage)
...
| 8. Full test suite passes | `pytest` | 1335 / 1335 tests pass |
...
# Expected: 1335 passed in ~N seconds
```
...
before any live dry run is attempted. The command is offline by default and does
not make external submissions. Manual reviewer signoff can be recorded with
...
| Metric | Definition | v0.27.0 Baseline |
|---|---|---|
...
### 12.4 Module Coverage Summary (v0.27.0)
