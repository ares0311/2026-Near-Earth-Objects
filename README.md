...
![Tests](https://img.shields.io/badge/tests-3454%20passing-brightgreen)
...
The pipeline follows the build order: `schemas` → `fetch` → `preprocess` → `detect` → `link` → `classify` → `orbit` → `score` → `alert` → `calibration`. Each stage consumes the immutable, typed output of all prior stages. As of v0.87.0, all ten pipeline modules plus background automation are complete, and the offline suite passes 3,454 tests with 2 live checks deselected. Real labeled ZTF data has trained Tier 1 and Tier 2, but no real survey field has completed the full pipeline; see `docs/PRODUCTION_READINESS.md` for the production gap register.
...
│
├── tests/                        # pytest suite (3454 passing; 2 live/integration deselected)
...
| 8. Full default test collection is stable | `pytest --collect-only` | 3454 passing tests; 2 live/integration tests deselected |
...
# Expected result: 3454 passing tests; 2 live/integration checks deselected.
```
...
| `alert.py` | — | 100% |
| `calibration.py` | — | 100% |
| **Total** | **3454 passing tests** | **100% target; verify with coverage run before release** |
...
| Area | Status | Notes |
|---|---|---|
| Core pipeline modules | **Complete** | All 10 modules: 3454 tests passing, 100% coverage target, ruff + mypy clean |
