...
![Version](https://img.shields.io/badge/version-0.90.66-informational)
...
v0.90.66, ZTF DR24 Gates Z1, Z2, Z4, Z5, Z6, and Z7 are closed with real
...
```
┌─────────────────────────────────────────────────────────────────────┐
│                    NEO DETECTION PIPELINE  v0.90.66                  │
└─────────────────────────────────────────────────────────────────────┘
...
│   ├── __init__.py               # Package version (0.90.66)
...
### 15.1 Current State Snapshot (v0.90.66)
...
|---|---|---|---|
| **A1** | Dataset manifest system | Training, scoring, evaluation, live-search, and evidence batches have manifest IDs, validation tests, source/license/role fields, and leakage caveats. | **Partially complete; schema and validator landed, pipeline citation still open** |
| **A2** | Candidate ledger | Candidate packets can be regenerated from a SQLite or parquet ledger containing source dataset ID, generator params, model versions, calibrated scores, review status, and regeneration command. | **Partially complete; SQLite schema/CLI landed, automatic pipeline ingest still open** |
...
| **A4** | Grouped splits and leakage checks | NEO train/eval splits are grouped by night, sky region, survey/instrument, and object ID; random splits remain diagnostic only. | **Partially complete; grouped leakage controls landed, model-builder adoption still open** |
| **A5** | Canonical regression evals | Known NEO detections, false link examples, injected moving-source controls, and review-packet examples produce sample-by-sample regression reports. | **Partially complete; suite runner and example contract landed, frozen production suites still open** |
