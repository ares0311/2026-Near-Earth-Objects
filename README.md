...
The pipeline follows the build order: `schemas` -> `fetch` -> `preprocess` -> `detect` -> `link` -> `classify` -> `orbit` -> `score` -> `alert` -> `calibration`. Each stage consumes the immutable, typed output of all prior stages. As of v0.90.5, all ten pipeline modules plus background automation are complete, all three ML tiers have trained weights, calibration KPIs have passed, T1/T2 production gaps are closed, and the discovery fetch layer is active for WISE/NEOWISE, DECam, and TESS. External MPC submission remains disabled until archival WISE/NEOWISE submission authority and candidate review gates are satisfied. See `docs/PRODUCTION_READINESS.md` for the authoritative production register.
...
│   ├── __init__.py               # Package version (0.90.5)
