...
now supersedes WISE/DECam/TESS as the primary discovery strategy. ZTF DR24
archival historical replay is the new primary discovery path; WISE/DECam/TESS
are secondary/paused (code and evidence preserved). See
...
v0.90.68, ZTF DR24 Gates Z1, Z2, Z4, Z5, Z6, and Z7 are closed with real
...
The pipeline follows the build order: `schemas` -> `fetch` -> `preprocess` -> `detect` -> `link` -> `classify` -> `orbit` -> `score` -> `alert` -> `calibration`. Each stage consumes the immutable, typed output of all prior stages. As of v0.90.68, the WISE/DECam/TESS path is a preserved secondary path with closed historical gates, while the primary development path is ZTF DR24 archival historical replay per `docs/MISSION.md` and `docs/neo_discovery_agent_brief.md`. ZTF DR24 gates are defined in `docs/ZTF_DR24_PRODUCTION_GATES.md`; Gates Z1, Z2, Z4, Z5, Z6, and Z7 are closed, and Gate Z3 is paused after real archived ZTF data confirmed pipeline mechanics but failed to confirm a single-object positive-control recovery across four candidate pairs. The current production-safe work is the Astrometrics A-gate sequence: production-run manifest citation, automatic candidate-ledger ingest, grouped splits, canonical evals, injection-recovery curves, and promotion reporting. External MPC submission remains fail-closed until a real candidate survives review and the applicable source/submission protocol is satisfied.
...
$$0.05 \;\text{arcsec hr}^{-1} \leq \dot\theta \leq 60 \;\text{arcsec hr}^{-1}$$
...
```
 ...
 └─────────────────────────────────────────────────────────────────────────┘
  ...
  └──────────────────┘            │  score.py → alert.py → calibration   │
   ...
   ├─ sample_tracklets.json ◀──────── batch_score.py             ├─ tier2_cnn.pt (benchmark)
   ├─ injection_recovery_            ├─ run_pipeline.py           ├─ tier2_cnn_v4.pt (promoted)
   │    baseline.json ◀─────────────  injection_recovery.py       └─ tier3_transformer.pt
...
Current work is focused on the ZTF DR24 archival historical-replay path:
bounded ingest, time-aware known-object exclusion, source-native detection
linking, auditable ranking, retrospective validation, and fail-closed package
drills. The WISE/DECam/TESS work remains preserved as secondary historical
evidence. MPC submission remains disabled until a real candidate survives
...
formed zero tracklets at both production and sensitivity thresholds; its fresh
control passed 20/20. A provenance-bound cross-batch analysis then combined
the expansion with run `017eb50381badb75`. IEO 147.53 supplied four retained
nights and 8,956 seed pairs but still zero production tracklets. All 70
cross-batch sensitivity fits are two-point/two-night pairs and not candidates.

That completed sparse expansion used the measured trio `20231003`,
`20231029`, `20240429`, the minimum-transfer solution among the 12
highest-exposure candidates that gave both sparse fields at least 80
central-box exposure rows. Raw archives remained unpersisted and external
submission remains prohibited. Further bulk replay requires a new, explicit
research decision.
...
Aten/IEO fields, three follow-up fields, and one post-ingest injection control.
Raw archives are not retained; the projected persistent addition is at most
1 GB. This authorizes search and internal review only—not MPC submission,
external alerts, or impact claims.
...
| ML model weights | **Trained; benchmark frozen; v4 internally promoted** | `tier2_cnn_v4` has two validated training manifests, a real grouped split, model-bound injection evidence, a 25-check canonical suite, passing real-data calibration, and a signed promotion report with all nine evidence artifacts passing. Jerome W. Lindsey III approved internal promotion under signoff ID `jlindsey-2026-07-12-tier2-cnn-v4`, accepting the disclosed 14/14 synthetic moving-source `stellar_artifact` result. `benchmark_cnn_v1` remains the immutable historical benchmark. |
...
| **M5b** | T1-A: Build cutout dataset + train Tier 2 CNN | `models/tier2_cnn.pt`; validation accuracy 91.3% | **Historically complete; frozen as `benchmark_cnn_v1`; not production-promoted** |
...
| **A7** | Calibration and promotion report | Any model promotion cites manifests, grouped splits, frozen evals, injection-recovery curves, calibration quantiles, false-discovery estimates, pretrained-model audits, benchmark model cards, and operator signoff where applicable. | **Complete for `tier2_cnn_v4`: all nine evidence artifacts pass and the signed report records operator approval. The operator packet preserves the 0/200 artifact result, passing real-data calibration, and accepted 14/14 synthetic moving-source `stellar_artifact` argmax tradeoff.** |
...
| **P45** | In progress | Dataset manifests and candidate ledger | Manifest schema/tests and SQLite ledger controls exist; remaining work is production-run manifest citation and automatic candidate-packet ledger ingest. |
| **P46** | In progress | Frozen CNN benchmark and grouped split checks | Existing CNN wrapped as `benchmark_cnn_v1`; grouped leakage validator added; remaining work is requiring split reports in model/eval promotion packets. |
| **P47** | In progress | Canonical evals and injection-recovery curves | Canonical eval runner and synthetic parameter curves landed; convert remaining image-level metadata into frozen policy-grade evals and recovery curves. |
...
- **CNN is not production-promoted yet**: The existing Tier 2 CNN is trained and frozen as `benchmark_cnn_v1`, but the new Astrometrics policy treats it as a benchmark until grouped split reports, canonical evals, injection-recovery curves, and a promotion report close.
