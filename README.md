...
![Version](https://img.shields.io/badge/version-0.90.8-informational)
...
The pipeline follows the build order: `schemas` -> `fetch` -> `preprocess` -> `detect` -> `link` -> `classify` -> `orbit` -> `score` -> `alert` -> `calibration`. Each stage consumes the immutable, typed output of all prior stages. As of v0.90.8, all ten pipeline modules plus background automation are complete, all three ML tiers have trained weights, calibration KPIs have passed, T1/T2 production gaps are closed, and the discovery fetch layer is active for WISE/NEOWISE, DECam, and TESS. External MPC submission remains disabled until archival WISE/NEOWISE submission authority and candidate review gates are satisfied. See `docs/PRODUCTION_READINESS.md` for the authoritative production register.
...
```
┌─────────────────────────────────────────────────────────────────────┐
│                    NEO DETECTION PIPELINE  v0.90.8                  │
└─────────────────────────────────────────────────────────────────────┘
...
│   ├── __init__.py               # Package version (0.90.8)
...
### 15.1 Current State Snapshot (v0.90.8)
...
| Production capability gates | **Partially closed** | P1 discovery-source positive control and P2 survey-native confidence policy (`docs/SURVEY_NATIVE_CONFIDENCE_POLICY.md`) are both closed. P3 no-submission package drill is the next open production gate. P4 MPC submission protocol and P5 operator go/no-go runbook are tracked in `docs/PRODUCTION_READINESS.md`. Actual discovery is an event gate, not a production prerequisite. |
