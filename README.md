...
The pipeline follows the build order: `schemas` → `fetch` → `preprocess` → `detect` → `link` → `classify` → `orbit` → `score` → `alert` → `calibration`. Each stage consumes the immutable, typed output of all prior stages. As of v0.30.0, all ten pipeline modules plus background automation are complete with 1340 tests, 100% code coverage, automated offline scheduling readiness, persisted readiness checks, no-network live dry-run planning, mock-only provider execution attempt logs, and validated injection-recovery baselines of 100% detection, link, and score rates on $n = 200$ synthetic NEO tracklets.
...
```
┌─────────────────────────────────────────────────────────────────────┐
│                    NEO DETECTION PIPELINE  v0.30.0                  │
└─────────────────────────────────────────────────────────────────────┘
...
│   ├── __init__.py               # Package version (0.30.0)
...
│
├── tests/                        # pytest suite (1340 tests; 100% coverage)
...
| 8. Full test suite passes | `pytest` | 1340 / 1340 tests pass |
...
# Expected: 1340 passed in ~N seconds
```
...
| Metric | Definition | v0.30.0 Baseline |
|---|---|---|
...
### 12.4 Module Coverage Summary (v0.30.0)
