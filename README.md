...
![Version](https://img.shields.io/badge/version-0.37.0-informational)
![License](https://img.shields.io/badge/license-Apache%202.0-green)
![Tests](https://img.shields.io/badge/tests-1358%20passing-brightgreen)
...
The pipeline follows the build order: `schemas` → `fetch` → `preprocess` → `detect` → `link` → `classify` → `orbit` → `score` → `alert` → `calibration`. Each stage consumes the immutable, typed output of all prior stages. As of v0.37.0, all ten pipeline modules plus background automation are complete with 1358 tests, 100% code coverage, automated offline scheduling readiness, provider-specific live readiness summaries, no-network live dry-run approval bundles, operator handoff exports, live policy contract validation, persisted readiness checks and approval bundles, no-network live dry-run planning, mock-only provider execution attempt logs, and validated injection-recovery baselines of 100% detection, link, and score rates on $n = 200$ synthetic NEO tracklets.
...
```
┌─────────────────────────────────────────────────────────────────────┐
│                    NEO DETECTION PIPELINE  v0.37.0                  │
└─────────────────────────────────────────────────────────────────────┘
...
│   ├── __init__.py               # Package version (0.37.0)
...
│
├── tests/                        # pytest suite (1358 tests; 100% coverage)
...
| 8. Full test suite passes | `pytest` | 1358 / 1358 tests pass |
...
# Expected: 1358 passed in ~N seconds
```
...
recorded from the review policy. Mock-only live dry-run provider execution
attempts can also be logged without contacting survey services. Injected
...
| Metric | Definition | v0.37.0 Baseline |
|---|---|---|
...
### 12.4 Module Coverage Summary (v0.37.0)
