...
| `Skills/hunter_cli.py export-cross-project-history` | Deliverable A of the three-repo Hunter history exchange: publishes **this** repo's own search history to `data_selection/hunter_prior_search_history_v1.json` (`schema_version=1`) so TechnoHunter/EXOHunter can consume it read-only. Derived from the committed `data_selection/target_priority_queue.csv` (the system of record; `hunter_state.sqlite` is empty). Native statuses map to the shared vocabulary (`null_result`→`no_signal`; thin-coverage→`no_data`, which correctly does **not** count as prior search); unmapped statuses, undated rows, and positionless rows all raise rather than publish something misleading. Refuses to write outside this repository (WS-01). Evidence: `docs/evidence/live/2026-08-01-neo-cross-project-history-publish.md` |
