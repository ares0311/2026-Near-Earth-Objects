"""Batch compute great-circle arc lengths for tracklets from JSON."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Batch compute great-circle arc lengths from tracklet JSON."
    )
    ...
    for entry in data:
        ...
        arc = compute_great_circle_arc(tracklet) if tracklet else 0.0
        rows.append({"object_id": entry.get("object_id", "unknown"), "arc_arcsec": arc})

    if args.json:
        print(json.dumps(rows, indent=2))
    else:
        print(f"{'object_id':<30} {'arc_arcsec':>14}")
        print("-" * 46)
        for row in rows:
            print(f"{row['object_id']:<30} {row['arc_arcsec']:>14.4f}")
