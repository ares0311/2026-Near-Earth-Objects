"""Get the faintest observation from a fetch-result JSON file.

Usage:
    python Skills/get_faintest_observation.py fetch_result.json [--json]

Reads a fetch-result JSON file and prints the observation with the faintest
(highest valid) magnitude using get_faintest_observation from fetch.py.
"""
import json
import sys
...
from fetch import get_faintest_observation
...
def main(argv: list[str]) -> None:
    ...
    if not paths:
        print("Usage: get_faintest_observation.py <fetch_result.json> [--json]",
              file=sys.stderr)
        sys.exit(1)
    ...
    obs = get_faintest_observation(fetch_result)

    if obs is None:
        if as_json:
            print(json.dumps({"faintest_observation": None}))
        else:
    ...
    if as_json:
        ...
        print(f"Faintest observation: {obs.obs_id}")
