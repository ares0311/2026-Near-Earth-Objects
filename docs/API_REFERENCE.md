...
| `Skills/audit_real_run.py` | Build fail-closed JSON/CSV audit packets for real pipeline runs; supports expected-known recovery manifests by pipeline ID or sky/time samples plus operator-review decisions |
| `Skills/build_recovery_manifest.py` | Build checkpointed MPC+Horizons expected-known manifests for T1-C recovery audits; no submission or impact claims |
| `Skills/select_survey_fields.py` | Select discovery or recovery fields; `--mode recovery` prioritizes known-object-rich non-Orion fields for T1-C recovery audits |
