...
**Current as of 2026-07-04 (v0.90.58)**: Gate Z6 (no-submission package
drill) and Gate Z7 (operator runbook update) are both **CLOSED**. Gate Z4
(auditable ranking baseline) tooling is built and offline-tested
(`Skills/evaluate_ranking_baseline.py`) and needs one operator run against
the real archived-negative checkpoints already on disk (no new download)
for a real-data closure result -- see the Gate Z4 row above for the exact
command. The Gate Z3 candidate-pair search below (designation 72966,
