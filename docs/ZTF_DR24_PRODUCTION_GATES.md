...
| MP5: Multi-night motion-consistency linking | **CLOSED (2026-07-17)** | `Skills/convert_pixel_extraction_to_observations.py` + `Skills/run_pixel_extraction_positive_control.py` reuse the real `preprocess()`+`link()` chain across multiple real nights (bypassing `detect()`'s WISE/DECam/TESS-only singleton gate, root-caused not guessed -- see the script's own module docstring). Validated on 3 independently-selected fields (9 real nights total): `min_observations=2` shows the expected crowded-field combinatorial explosion (200, 200, and 71 tracklets); the real `min_observations=3` default collapses fields 1/2 to 2 and 5 survivors respectively (all fail MP4's independent PSF check), and field 3 to **zero** survivors outright. See `docs/evidence/live/2026-07-17-ztf-dr24-multi-night-linking-first-test.md`, `docs/evidence/live/2026-07-17-ztf-dr24-selected-field-linking-test.md`, and `docs/evidence/live/2026-07-17-ztf-dr24-third-field-linking-test.md`. |
