...
| Z4: Auditable ranking baseline | **Tooling built and offline-tested (2026-07-04); pending one operator run for a real-data closure result** | `Skills/evaluate_ranking_baseline.py` evaluates a handcrafted-feature logistic-regression baseline (`classify.extract_features`/`features_to_vector`, out-of-fold stratified k-fold, never scored on data it was fit on) against real archived negative tracklets (reusing the Gate Z6-evidenced 20220817/20220819 and 20210106/20210111 checkpoints already on disk -- no new download) and synthetic positive tracklets (the established injection generator). Reports recall@K, purity@K, calibration error (Brier/ECE/log-loss), false-positive review burden, and an ablation vs. a naive real-bogus-only baseline. 10 offline tests pass against synthetic fixtures. Not yet run against the operator's real checkpoint files (this sandbox has no access to `Logs/pipeline_runs/ztf_alert_archive_ingest/*.json`, which are local-only and never committed). |
| Z5: Retrospective validation | **Tooling built and offline-tested (2026-07-04); pending one operator run for a real-data closure result** | `Skills/evaluate_retrospective_validation.py` evaluates review packets against the MPC catalog as queried today (legitimately after the replay window -- this is retrospective validation, not replay-time leakage), bucketing each candidate into `recovered_known_object`, `later_confirmed_object`, `artifact`, or `unresolved_candidate` using the already-real `check_candidates_against_mpc` live cross-match plus each packet's own `known_object_score`. Does not depend on JPL SBDB's not-yet-live-verified `first_obs` field. 8 offline tests pass against injected fixtures (no real network call made in tests). Not yet run against real review packets + a live MPC connection (this sandbox has neither the operator's real checkpoint files nor live network access). |
...
**Current as of 2026-07-04 (v0.90.59)**: Gate Z6 (no-submission package
drill) and Gate Z7 (operator runbook update) are both **CLOSED**. Gate Z4
(auditable ranking baseline) and Gate Z5 (retrospective validation)
tooling are both built and offline-tested
(`Skills/evaluate_ranking_baseline.py`,
`Skills/evaluate_retrospective_validation.py`) and each need one operator
run for a real-data closure result -- see their rows above for the exact
commands. Both reuse data already on disk or already-verified live
mechanisms; neither requires a new archival download. The Gate Z3
