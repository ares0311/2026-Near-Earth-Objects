...
| MP5: Multi-night motion-consistency linking | **CLOSED (2026-07-17)** | `Skills/convert_pixel_extraction_to_observations.py` + `Skills/run_pixel_extraction_positive_control.py` reuse the real `preprocess()`+`link()` chain across multiple real nights (bypassing `detect()`'s WISE/DECam/TESS-only singleton gate, root-caused not guessed -- see the script's own module docstring). Validated on 5 independently-selected fields (15 real nights total; one additional candidate field correctly skipped for insufficient coverage before consuming download budget): `min_observations=2` shows the expected crowded-field combinatorial explosion (200, 200, 71, 200, and 95 tracklets); the real `min_observations=3` default gives 2, 5, 0, 6, and 2 survivors respectively -- every survivor across all five fields fails MP4's independent PSF check and/or adversarial review. See `docs/evidence/live/2026-07-17-ztf-dr24-multi-night-linking-first-test.md`, `docs/evidence/live/2026-07-17-ztf-dr24-selected-field-linking-test.md`, `docs/evidence/live/2026-07-17-ztf-dr24-third-field-linking-test.md`, `docs/evidence/live/2026-07-18-ztf-dr24-fourth-field-linking-test.md`, and `docs/evidence/live/2026-07-19-ztf-dr24-sixth-field-linking-test.md`. |
