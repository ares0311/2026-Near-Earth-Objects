...
| 9 | 2026-06-28 | Root cause identified and patch validated locally: WISE fetch queried broad static point-source rows and detect required same-night pairs before multi-night linking; targeted tests `80 passed`, ruff clean, mypy clean | Merge WISE moving-source prefilter + discovery-archive singleton linking, then run a smaller WISE dry-run diagnostic from `main` |
