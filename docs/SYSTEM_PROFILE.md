...
|---|---|---|
| UW public ZTF nightly alert archives | 6 concurrent nightly streams | Clean 2026-07-14 portfolio run `0b381aac323c0f28`: six disjoint nights, 38.98 GB streamed in 9m48s, zero service/rate-limit errors. Coverage-qualified run `017eb50381badb75` independently completed four streams, 26.67 GB in 10m36s, also without service/rate-limit errors. This supports at most the measured six streams, not 36. |
| IRSA ZTF science-image metadata | 10 concurrent bounded queries | Clean 2026-07-14 coverage run `9a9e148f570d162b` first supported 6 (six disjoint fields, 6,534 rows, 10 seconds, zero errors). Re-probed 2026-07-18 progressively at 2/4/6/10 concurrent (genuinely uncached RA/Dec, verified against every existing checkpoint to rule out cache-hit false positives): all 10 succeeded at the highest level tested, with no worse tail latency than at 6 (max 18s vs. max 26s) -- no ceiling found yet at 10. See `docs/evidence/live/2026-07-18-irsa-concurrency-probe.md`. |
| IRSA ZTF DR24 pixel-product downloads (difference image + mask + PSF, ~7-19MB each) | **3 concurrent — conservative starting point, NOT a confirmed ceiling** | 2026-07-18 probe: 4 and 6 concurrent both completed with zero hard errors (no 429/503/timeout), but with real, worsening latency vs. the ~2-4s serial baseline (4 concurrent: 23-33s; 6 concurrent: 10-47s). **A confound was identified during the same session** (operator's local connection changed mid-probe, boarding a flight) that could fully explain the same symptom independent of server-side behavior -- this endpoint's result is therefore NOT held to the same "confirmed safe" bar as the metadata row above. Do not exceed 3 concurrent for this endpoint until a future session re-runs the probe on a stable connection and upgrades this row. See `docs/evidence/live/2026-07-18-irsa-concurrency-probe.md`. |
...
| 2026-07-14 | 6 outer file shards x 6 pytest-xdist workers; native threads pinned to 1 | Clean v0.90.98 broad run: 1,950 tests passed in 29 s; six isolated coverage datasets combined without errors; 5,447 source statements at 100% coverage. |
